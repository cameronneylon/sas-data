...
class TestSasData(unittest.TestCase):
	...
	self.assertTrue(len(self.test_data_zeros) == 10)
	...
	self.assertEqual(self.zero_to_nine, test_add.q)
        #self.assertRaises(TypeError, self.test_data_ranges +
                            #self.test_string)
        #test_add = self.test_data_ranges + self.test_string
	#self.assertEqual(test_add, NotImplemented)

    def test_sub(self):
        test_sub18 = SasData(self.zero_to_nine, self.eighteen_to_zero)
	test_sub = test_sub18 - self.test_data_ranges
	self.assertEqual(self.nine_to_zero, test_sub.i)
	self.assertEqual(self.zero_to_nine, test_sub.q)

        test_sub = SasData(self.zero_to_nine, range(18, 8, -1))
	test_sub = test_sub - 5
	self.assertEqual(self.thirteen_to_four, test_sub.i)
	self.assertEqual(self.zero_to_nine, test_sub.q)
        #self.assertRaises(TypeError, self.test_data_ranges.__sub__,
                            #self.test_string)
        #test_sub = self.test_data_ranges - self.test_string             
        #self.assertEqual(test_sub, NotImplemented)
